...
async function createPlaylist() {
  await fetch(
    `https://api.spotify.com/v1/users/${user_id}/playlists?offset=0&limit=1`,
    {
      body: `{"name":"${playlistName}","description":"Generated by SoundPath","public":false}`,
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${access_token}`,
        "Content-Type": "application/json",
...
// ================================================================= //
// =        Get Playlist URIs & Send To Spotify User Playlist      = //
// ================================================================= //

async function sendSongs() {
  let uriString = "";
  function getUris() {
    // For Loop to Populate Songs String
    for (let i = 0; i < songsArray.length; i++) {
      const thisSong = songsArray[i].uri;
      uriString += `${thisSong},`;
    }

    await fetch(
      `https://api.spotify.com/v1/playlists/${playlistId}/tracks?uris=${uriString
        .replace(":", "%3A")
        .replace(",", "%2C")}`,
      {
        headers: {
